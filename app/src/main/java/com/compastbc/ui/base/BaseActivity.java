...
import com.compastbc.data.db.model.ActivityLog;
import com.compastbc.data.db.model.AttendanceLog;
import com.compastbc.data.db.model.Beneficiary;
import com.compastbc.data.db.model.BeneficiaryDao;
import com.compastbc.data.db.model.BeneficiaryGroups;
import com.compastbc.data.db.model.BlockCards;
import com.compastbc.data.db.model.Categories;
import com.compastbc.data.db.model.ExceptionLog;
import com.compastbc.data.db.model.Language;
...
import com.compastbc.data.db.model.TopupLogsDao;
import com.compastbc.data.db.model.TransactionsDao;
import com.compastbc.data.db.model.Users;
import com.compastbc.data.db.model.UsersDao;
import com.compastbc.data.db.model.Vouchers;
import com.compastbc.data.network.Webservices;
...
import com.compastbc.utils.CommonUtils;
import com.compastbc.utils.NetworkUtils;
import com.compastbc.utils.print.BlueToothCallback;
import com.compastbc.utils.print.PrintServices;
import com.compastbc.utils.print.ReportPrintCallback;
...
public abstract class BaseActivity extends AppCompatActivity implements MvpView, BaseFragment.Callback {
    ...
    private AppCompatActivity activity = this;    private final BroadcastReceiver onDownloadComplete = new BroadcastReceiver() {
        @Override
    ...
    protected void setLatLng(@NonNull Location location) {
        Compas.LATITUDE = location.getLatitude();
        Compas.LONGITUDE = location.getLongitude();
        AppLogger.e("Location", String.valueOf(Compas.LATITUDE));

        /*if (address.isEmpty()) {
            address = getAddressFromLatLng(Compas.LATITUDE, Compas.LONGITUDE);
    ...
    protected void onResume() {
        ...
        if (!getDataManager().getConfigurableParameterDetail().isOnline() && getDataManager().getConfigurableParameterDetail().isBiometric()) {
            ...
            if (getIp() == null)
                initDeviceConnection(false, null);
        }
    }
                                ...
                                if (!getDataManager().getConfigurableParameterDetail().isOnline()) {
                                    ...
                                    if (programmes.length() > 0) {
                                        ...
                                        for (int i = 0; i < programmes.length(); i++) {
                                            ...
                                            JSONObject programObj = programmes.getJSONObject(i);
                                            programmes_table.setProgramId(programObj.getString("programmeId"));
                                            programmes_table.setProgramName(programObj.getString("programmeName"));
                                            programmes_table.setProductId(programObj.getString("productId"));
                                            if (programObj.has("programCurrency"))
                                                programmes_table.setProgramCurrency(programObj.getString("programCurrency"));

                                            //Note: Please dont change order in parsing 1. commodity->categories->vouchers
                                            if (programObj.has("commodity")) {
                                                JSONArray commodityArray = programObj.getJSONArray("commodity");

                                                List<Services> servicesList = new ArrayList<>();
                                                for (int s = 0; s < commodityArray.length(); s++) {
                                                    Services service = new Services();
                                                    JSONObject commodityObj = commodityArray.getJSONObject(s);

                                                    String[] image = commodityObj.getString("image").split(",");
                                                    service.setServiceImage(image[1]);
                                                    service.setServiceCode(commodityObj.getString("productCode"));
                                                    service.setServiceId(commodityObj.getString("productId"));
                                                    service.setServiceName(commodityObj.getString("productName"));
                                                    service.setServiceType(commodityObj.getString("productType"));
                                                    service.setCategoryId(commodityObj.getString("categoryId"));
                                                    if (commodityObj.getString("productType").equalsIgnoreCase("commodity")) {
                                                        service.setLocationId(commodityObj.getString("locationId"));
                                                        JSONArray servicePrices = commodityObj.getJSONArray("priceDetails");
                                                        ...
                                                        for (int j = 0; j < servicePrices.length(); j++) {
                                                            ...
                                                            servicePrices1.setServiceId(commodityObj.getString("productId"));
                                            ...
                                            if (programObj.has("categories")) {
                                                JSONArray categoriesArray = programObj.getJSONArray("categories");
                                                List<Categories> categoriesList = new ArrayList<>();
                                                for (int c = 0; c < categoriesArray.length(); c++) {
                                                    Categories category = new Categories();
                                                    JSONObject categoryObj = categoriesArray.getJSONObject(c);
                                                    category.setCategoryId(categoryObj.getString("categoryId"));
                                                    category.setCategoryName(categoryObj.getString("categoryName"));
                                                    category.setProductId(categoryObj.getString("productId"));
                                                    categoriesList.add(category);
                                                }
                                                getDataManager().getDaoSession().getCategoriesDao().insertInTx(categoriesList);
                                            }

                                            if (programObj.has("vouchers")) {
                                                JSONArray vouchersArray = programObj.getJSONArray("vouchers");
                                                List<Vouchers> vouchersList = new ArrayList<>();
                                                for (int j = 0; j < vouchersArray.length(); j++) {
                                                    Vouchers voucher_table = new Vouchers();
                                                    JSONObject voucherObj = vouchersArray.getJSONObject(j);
                                                    JSONArray products = voucherObj.getJSONArray("products");
                                                    voucher_table.setVoucherId(voucherObj.getString("voucherId"));
                                                    voucher_table.setVoucherName(voucherObj.getString("voucherName"));
                                                    voucher_table.setProgramId(programObj.getString("programmeId"));
                                                    List<ServiceDetails> serviceDetailsList = new ArrayList<>();
                                                    for (int k = 0; k < products.length(); k++) {
                                                        ServiceDetails service_detail_table = new ServiceDetails();
                                                        JSONObject productObj = products.getJSONObject(k);
                                                        service_detail_table.setServiceId(productObj.getString("serviceId"));
                                                        service_detail_table.setVoucherId(voucherObj.getString("voucherId"));
                                                        service_detail_table.setProgramId(programObj.getString("programmeId"));
                                                        serviceDetailsList.add(service_detail_table);

                                                        Services serviceBean = servicesDao.queryBuilder().where(ServicesDao.Properties.ServiceId.eq(service_detail_table.getServiceId())).limit(1).unique();
                                                        if (serviceBean != null) {
                                                            if (!productObj.isNull("maxQuantity"))
                                                                serviceBean.setMaxQuantity(productObj.getDouble("maxQuantity"));
                                                            if (!productObj.isNull("maxQuantityBenf"))
                                                                serviceBean.setMaxQuantityBenf(productObj.getDouble("maxQuantityBenf"));
    ...
    public synchronized void createLog(String activityName, String action) {
        if (getDataManager().getConfigurableParameterDetail().getActivityLog()) {
            ...
            al.setLatitude(Compas.LATITUDE);
            al.setLongitude(Compas.LONGITUDE);
    ...
    public void doCheckAppVersion(final boolean isShowMsg, final AppUpdateCallback appUpdateCallback) {
        ...
        if (isNetworkConnected()) {
            if (checkDBForSync()) {
                getDataManager().doUpdateApplication().enqueue(new Callback<ResponseBody>() {
                    ...
                    public void onResponse(@NonNull Call<ResponseBody> call, @NonNull Response<ResponseBody> response) {
                        ...
                        try {
                            if (response.code() == 200) {
                                ...
                                if (!appName.isEmpty() && !appVersion.isEmpty()) {
                                    ...
                                    if (flag > 0) {
                                        ...
                                        sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.alert), getString(R.string.app_update_alert))
                                                .setConfirmText(getString(R.string.dialog_ok))
                                                .setConfirmClickListener(sweetAlertDialog -> {
                                                    sweetAlertDialog.dismissWithAnimation();
                                                    showAppUpdateUI(finalAppName, bundle);
                                                })
                                        ...
                                        if (isShowMsg) {
                                            sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.alert), getString(R.string.alert_latest_ver_msg))
                                ...
                                if (isShowMsg) {
                                    sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.error), getString(R.string.UnauthorisedAccess))
                                            .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
                                }
                            } else if (response.code() == 404) {
                                if (appUpdateCallback != null) appUpdateCallback.onFail();
                                if (isShowMsg) {
                                    sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.error), getString(R.string.error_404))
                                            .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
                                }
                            } else {
                                if (appUpdateCallback != null) appUpdateCallback.onFail();
                            ...
                            e.printStackTrace();
                            if (isShowMsg) {
                                sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.error), getString(R.string.some_error))
                                        .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
                            }
                        }
                    }
                ...
                hideLoading();
                sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.alert), getString(R.string.Pleasesyncdatawithserver))
                        .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
            }
        }
    }

    public void showAppUpdateUI(String finalAppName, Bundle bundle) {
        AppUpdateDialog.newInstance(bundle, () -> {
            String finalAppPath = Webservices.getApkDownloadPath(finalAppName);
            AppLogger.e("AppUpdate", finalAppPath);
            if (finalAppPath.contains(".apk"))
                beginAppDownload(finalAppPath, finalAppName);
            else {
                sweetAlertDialog(SweetAlertDialog.WARNING_TYPE, getString(R.string.alert), getString(R.string.alert_apk_issue))
                        .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
            }
    ...
    private void beginAppDownload(String appPath, String finalAppName) {
        /*
        Create a DownloadManager.Request with all the information necessary to start the download
         */
        String destination = getActivity().getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS) + "/";
    ...
    public void initDeviceConnection(boolean isSendData, ArrayList<String> values) {
        try {
            ip = null;
            ...
            if (ip != null) {
                ...
                establishDeviceConnection(isSendData, values);
            } /*else {
    ...
    public void doSendBenfData(ArrayList<String> values) {
        if (values.isEmpty()) {
            ...
            try {
                handler.post(() -> {
                    if (!getActivity().isFinishing()) {
                        sweetAlertDialog(SweetAlertDialog.SUCCESS_TYPE, getString(R.string.success), getString(R.string.data_receive_success))
                                .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
                    }
                });
            } catch (Exception e) {
    ...
    private synchronized void parseBenfData(String medd, DatagramPacket recv) {
        try {
            ...
            if (beneficiary != null) {
                ...
                handler.post(() -> {
                    if (!getActivity().isFinishing()) {
                        sweetAlertDialog(SweetAlertDialog.ERROR_TYPE, getString(R.string.fail), getString(R.string.sync_bnf_data_error))
                                .setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
                    }
                });
            }
    ...
    public void establishDeviceConnection(boolean isSendData, ArrayList<String> values) {
        if (wifiManager == null)
        ...
        ReceiverData recvMsgThread = new ReceiverData();//Data Messages receiver client
        recvMsgThread.execute((Void) null);


        if (isSendData)
            doSendBenfData(values);
    }
    ...
    private class ReceiverData extends AsyncTask<Void, Void, Boolean> {//Asynchronous type class to receive messages
                        ...
                        try {
                            ...
                            if (medd.contains("success")) {
                                try {
                                    handler.post(() -> {
                                        if (!getActivity().isFinishing()) {
                                            sweetAlertDialog(SweetAlertDialog.SUCCESS_TYPE, getString(R.string.success), getString(R.string.dataSend)).setConfirmClickListener(sweetAlertDialog -> {
                                                sweetAlertDialog.dismissWithAnimation();
                                ...
                                try {
                                    handler.post(() -> {
                                        if (!getActivity().isFinishing()) {
                                            sweetAlertDialog(SweetAlertDialog.ERROR_TYPE, getString(R.string.fail), getString(R.string.somethingWentWrong)).setConfirmClickListener(SweetAlertDialog::dismissWithAnimation).show();
                                        }
                                    });
                                } catch (Exception e) {
                                    e.printStackTrace();
