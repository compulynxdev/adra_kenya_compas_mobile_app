...
public class CardBalancePresenter<V extends CardBalanceMvpView> extends BasePresenter<V>
    ...
    private List<Topups> topupsList = new ArrayList<>();
        ...
        if (status.size() > 2) {
            try {
                ...
                if (object.length() > 0) {
                    ...
                    List<com.compastbc.data.db.model.Topups> topupsList = getDataManager().getDaoSession().getTopupsDao().queryBuilder().where(TopupsDao.Properties.CardNumber.eq(cardNo)).list();
