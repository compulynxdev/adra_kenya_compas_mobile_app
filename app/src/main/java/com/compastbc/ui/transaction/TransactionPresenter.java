...
import com.compastbc.data.db.model.BeneficiaryBioDao;
import com.compastbc.data.db.model.BeneficiaryDao;
import com.compastbc.data.db.model.BlockCards;
import com.compastbc.data.db.model.BlockCardsDao;
import com.compastbc.data.db.model.NFCCardData;
import com.compastbc.data.db.model.NFCCardDataDao;
...
import com.compastbc.data.db.model.TopupLogs;
...
import com.compastbc.utils.nfc.NFCReadListDataListener;
...
public class TransactionPresenter<V extends TransactionMvpView> extends BasePresenter<V>
    ...
    private final NFCReader nfcReader= null;
...
//        nfcReader = NFCReader.getInstance(activity);
    ...
    @Override
    public void readQrCode(String result) {
        try {
            JSONObject jsonObject = new JSONObject(result);

            identityNo = jsonObject.getString("rationalNo");
            beneficiaryName = jsonObject.getString("name");
            cardNo = jsonObject.getString("cardNo");
            cardPin = jsonObject.getString("pin");
            ...
            e.printStackTrace();
            getMvpView().showMessage(R.string.invalid_qr_code);
        }
    }
    ...
    public void qrFindTopups(String cardNo) {
        List<com.compastbc.data.db.model.Topups> topupsList = getDataManager().getDaoSession().getTopupsDao().queryBuilder()
        ...
        if (!topupsList.isEmpty()) {
            try {
                ...
                QrtopUp(topupsList, new JSONArray());
    ...
    private void QrtopUp(List<com.compastbc.data.db.model.Topups> topupsList, JSONArray originalProgramArray) {
        ...
        try {
            if (originalProgramArray.length() == 0) {
                ...
                for (int i = 0; i < topupsList.size(); i++) {
                    com.compastbc.data.db.model.Topups topupBean = topupsList.get(i);
    ...
    private void topUpCard(List<com.compastbc.data.db.model.Topups> topupsList, JSONArray originalProgramArray) {
        JSONObject topupJsonObject = new JSONObject();
        String voucherOldValue = "0", voucherOldNo = "0";
    ...
    private void updateTopupLogs(com.compastbc.data.db.model.Topups topupBean, String voucherOldValue, String voucherOldNo) {
