...
import com.compastbc.ui.transaction.services.cashdialog.CashDialog;
import com.compastbc.ui.transaction.services.servicedialog.ServiceDialog;
...
public class ServiceActivity extends BaseActivity implements ServiceMvpView, ServiceAdapter.ItemClickListener {
    ...
    public void showDialog(List<ServicePrices> servicePrices) {
        ServiceDialog.newInstance(services, servicePrices,
                () -> tvVoucherValue.setText(getDataManager().getCurrency().concat(" ")
                        .concat(String.format(Locale.getDefault(), "%.2f", Double.parseDouble(getDataManager().getTopupDetails().getVouchervalue()))))).show(getSupportFragmentManager(), "Service Dialog");
    }
